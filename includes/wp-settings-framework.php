...
if ( ! class_exists( 'wpt_WordPressSettingsFramework' ) ) {
	/**
	 * WordPressSettingsFramework class
	 */
	class wpt_WordPressSettingsFramework {
		...
		/**
		 * Displays the "section_description" if speicified in $wptsf_settings
		 *
		 * @param array callback args from add_settings_section()
		 */
		function section_intro( $args ) {
			global $wptsf_settings;
			if ( ! empty( $wptsf_settings ) ) {
				foreach ( $wptsf_settings as $section ) {
		...
		/**
		 * Processes $wptsf_settings and adds the sections and fields via the WordPress settings API
		 */
		function process_settings() {
			global $wptsf_settings;
			if ( ! empty( $wptsf_settings ) ) {
				usort( $wptsf_settings, array( &$this, 'sort_array' ) );
				foreach ( $wptsf_settings as $section ) {
		...
		/**
		 * Usort callback. Sorts $wptsf_settings by "section_order"
		 *
		...
		function generate_setting( $args ) {
			...
			switch ( $type ) {
				...
				case 'radio':
					$val = esc_html( esc_attr( $val ) );
					foreach ( $choices as $ckey => $cval ) {
						echo '<span><input type="radio" name="' . $this->option_group . '_settings[' . $el_id . ']" id="' . $el_id . '_' . $ckey . '" value="' . $ckey . '" class="' . $class . '"' . ( ( $ckey == $val ) ? ' checked="checked"' : '' ) . ' /> ' . $cval . '</span><br />';
					}
					if ( $desc ) {
						echo '<p class="description">' . $desc . '</p>';
					}
				...
				case 'checkbox':
					...
					echo '<span><input type="checkbox" name="' . $this->option_group . '_settings[' . $el_id . ']" id="' . $el_id . '" value="1" class="' . $class . '"' . ( ( $val ) ? ' checked="checked"' : '' ) . ' /> ' . $desc . '</span>';
					break;
				case 'checkboxes':
					foreach ( $choices as $ckey => $cval ) {
						$val = '';
						...
						echo '<span><input type="checkbox" name="' . $this->option_group . '_settings[' . $el_id . '_' . $ckey . ']" id="' . $el_id . '_' . $ckey . '" value="' . $ckey . '" class="' . $class . '"' . ( ( $ckey == $val ) ? ' checked="checked"' : '' ) . ' /> ' . $cval . '</span><br />';
					}
					if ( $desc ) {
						echo '<p class="description">' . $desc . '</p>';
					}
